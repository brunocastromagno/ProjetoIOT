...
def adicionar_registro(table, *values):
    try:
        ...
        print(f"Erro ao adicionar registro em {table}: {e}")
...
def gerar_relatorio(table, field, where_clause):
    try:
        ...
        print(f"Erro ao gerar relatório de {table}: {e}")
        return None
...
def criar_janela_adicionar_registro(table, fields):
    try:
        ...
        print(f"Erro ao criar janela para adicionar registro em {table}: {e}")
...
def criar_janela_gerar_relatorio(table, field):
    try:
        ...
        print(f"Erro ao criar janela para gerar relatório de {table}: {e}")
...
def rastrear_tag(numero_serie):
    try:
        ...
        print(f"Erro ao rastrear a TAG: {e}")
        return None
...
def rastrear_equipamento():
    try:
        ...
        print(f"Erro ao rastrear o equipamento: {e}")
...
tk.Label(login_frame, text="Usuário:").grid(row=0, column=0, padx=10)
entry_username = tk.Entry(login_frame)
entry_username.grid(row=0, column=1)

tk.Label(login_frame, text="Senha:").grid(row=1, column=0, padx=10)
